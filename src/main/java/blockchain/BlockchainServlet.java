...
public class BlockchainServlet extends HttpServlet {
	...
	//public static ArrayList<Block> blockchain = new ArrayList<Block>(); // 블록체인 저장을 위한 ArrayList
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (symbol != null && voteId != null && voteTime != null && email != null) {
            // 출력
            //System.out.println("보트 ID: " + voteId);
            //System.out.println("투표 시간: " + voteTime);
            //System.out.println("사용자 이메일: " + email);
            //System.out.println("투표한 후보 기호: " + symbol);
        	
        	// 해당 투표 ID에 대한 블록체인을 가져오거나 새로 생성
        	//ArrayList<Block> blockchain = blockchainMap.getOrDefault(voteId, new ArrayList<>());

            // TEST용: 블록체인에 기존 블록이 있는 경우, 가장 최근 블록의 이메일을 수정
            /*if (!blockchain.isEmpty()) {
                Block lastBlock = blockchain.get(blockchain.size() - 1); // 가장 최근 블록
                lastBlock.getVoteData().setVoterEmail("false@test.com"); // 이메일 변경
            }*/
            ...
            //Block newBlock = new Block("block " + blockNumber, voteData, blockchain.get(blockchain.size()-1).hash);
    ...
    private void saveBlockchainToFile(String voteId, ArrayList<Block> blockchain) {
        try {
            String blockchainJson = new GsonBuilder().setPrettyPrinting().create().toJson(blockchain);
            Files.write(Paths.get(voteId + "_blockchain.json"), blockchainJson.getBytes());
            System.out.println("블록체인 파일이 저장되었습니다: " + voteId + "_blockchain.json");
        } catch (IOException e) {
    ...
    private void loadBlockchainFromFile(String voteId) {
        ...
        } catch (IOException e) { // 파일 없는 경우에도 이 오류 생김
