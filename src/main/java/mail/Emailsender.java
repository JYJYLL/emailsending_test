...
public class Emailsender {
	...
	private static String adminpw;
	 ...
	 static {
	        ...
	        adminmail = config.getEmail();
	        adminpw = config.getEmailPW();
	    }
	...
	public static String generateAndSendEmail(String mail) throws AddressException, MessagingException {
		...
		//generateMailMessage.addRecipient(Message.RecipientType.CC, new InternetAddress("")); // 참조 수신자 설정
		...
		transport.connect("smtp.gmail.com", adminmail, adminpw);
		transport.sendMessage(generateMailMessage, generateMailMessage.getAllRecipients()); // 이메일 전송
		transport.close(); // 전송 완료 후 연결 종료
		
		return otp; // 생성된 OTP 반환
	}
	
	// 이메일을 생성하고 전송 (비번재설정용)
	public static String generateAndSendEmailforPW(String mail) throws AddressException, MessagingException {
		...
		try {
            ...
            String emailBody = "<html>" +
            	    ...
            	    "<p>안녕하세요,</p>" +
            	    "<p>저희 서비스를 이용해 주셔서 감사합니다.</p>" +
            	    "<p>비밀번호 재설정을 위한 일회성 비밀번호(OTP)는 다음과 같습니다:</p>" +
            	    "<p class='otp'>" + otp + "</p>" +
            	    "<p>이 OTP를 사용하여 비밀번호 재설정을 완료해 주세요.</p>" +
            	    "<p>만약 비밀번호 재설정을 요청하지 않으셨다면, </p>" +
            	    "<p>계정 보안을 위해 즉시 고객 지원팀에 문의하시기 바랍니다.</p>" +
            	    "</div>" +
            	    "<div class='footer'>" +
            	    "<p>감사합니다,<br>JY</p>" +
            	    "</div>" +
